...
class TransformerEncoderLayer(nn.Module):
    ...
    #实现了一个带有位置编码的自注意力机制 (self-attention) 和两个后续的前馈神经网络层 (feed-forward layers)
    #返回经过自注意力机制和前馈神经网络处理后的特征图 src。
    #norm在attention之后
    def forward_post(self,
                     ...
                     src_key_padding_mask: Optional[Tensor] = None,
                     pos: Optional[Tensor] = None):
        q = k = self.with_pos_embed(src, pos) #相加
        #经过注意力机制处理后的特征图。self.self_attn代表一个自注意力层。
